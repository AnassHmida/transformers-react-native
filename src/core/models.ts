...
import { executionProviders, ONNX } from '../backends/onnx.ts';
import { medianFilter } from '../../index.ts';
...
async function constructSession(pretrained_model_name_or_path, fileName, options) {
    ...
    console.log('Constructing session for:', pretrained_model_name_or_path, fileName);
    console.log('Execution providers:', executionProviders);
    console.log('Session options:', options.session_options);
    ...
    if (IS_BROWSER || !env.useFS || !env.allowLocalModels || env.useCustomCache) {
        console.log('Using buffer method for session creation');
        try {
            const getModelFileStart = Date.now();
            let buffer = await getModelFile(pretrained_model_name_or_path, modelFileName, true, options);
            console.log(`getModelFile took ${Date.now() - getModelFileStart}ms`);
            console.log('Buffer size:', buffer.byteLength);

            const sessionCreateStart = Date.now();
            const session = await InferenceSession.create(buffer, sessionOptions);
            console.log(`InferenceSession.create took ${Date.now() - sessionCreateStart}ms`);
            console.log('Session created successfully');
            return session;
        } catch (error) {
            console.error('Error creating session with buffer:', error);
            throw error;
        }
    } else {
        console.log('Using file path method for session creation');
        try {
            const getModelPathStart = Date.now();
            let modelPath = await getModelPath(pretrained_model_name_or_path, modelFileName, true, options);
            console.log(`getModelPath took ${Date.now() - getModelPathStart}ms`);
            ...
            console.error('Error creating session with file path:', error);
            throw error;
        }
    }
    console.log(`Total constructSession time: ${Date.now() - startTime}ms`);
}
